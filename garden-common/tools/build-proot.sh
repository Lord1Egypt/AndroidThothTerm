...
#
# Garden builds this once per edition, because the runtime directory is
# compiled into libandroid-shmem and PRoot and it contains the application id.
#
# Environment:
#   GARDEN_APPLICATION_ID   e.g. com.thothterm.debian (required)
#   GARDEN_JNI_DIR          where libproot.so and libproot_loader.so go (required)
#   GARDEN_RUNTIME_ASSETS   where libtalloc.so.2 and libandroid-shmem.so go (required)
#   GARDEN_PROOT_BUILD_DIR  scratch directory (default: a temp dir)
#
# Outputs, named lib*.so so Android extracts them into nativeLibraryDir with the
# executable bit:
#   $GARDEN_JNI_DIR/libproot.so            (the proot executable)
#   $GARDEN_JNI_DIR/libproot_loader.so     (the PRoot loader)
#   $GARDEN_RUNTIME_ASSETS/libtalloc.so.2
#   $GARDEN_RUNTIME_ASSETS/libandroid-shmem.so
#
# Requires ANDROID_NDK_HOME (or ANDROID_SDK_ROOT/ANDROID_HOME with an
# ndk/<version> directory).
set -eu
...
APP_ID="${GARDEN_APPLICATION_ID:?set GARDEN_APPLICATION_ID}"
# Must match GardenPaths.RUNTIME_DIR on the Java side.
RUNTIME_DIR="/data/data/${APP_ID}/files/garden/runtime"
...
BUILD_DIR="${GARDEN_PROOT_BUILD_DIR:-$(mktemp -d)}"
JNI_DIR="${GARDEN_JNI_DIR:?set GARDEN_JNI_DIR}"
RUNTIME_ASSETS="${GARDEN_RUNTIME_ASSETS:?set GARDEN_RUNTIME_ASSETS}"
...
ndk_root() {
    ...
    if [ -n "${GARDEN_NDK_VERSION:-}" ] \
        && [ -d "$_sdk/ndk/$GARDEN_NDK_VERSION" ]; then
        echo "$_sdk/ndk/$GARDEN_NDK_VERSION"; return 0
    fi
